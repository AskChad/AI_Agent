...
async function deployMigration() {
  console.log('📦 Deploying Google Docs Integration Migration...\n');

  const migrationPath = path.join(__dirname, '..', 'database', 'migrations', '007_google_docs_integration.sql');
  ...
  try {
    ...
    if (response.statusCode === 200 || response.statusCode === 201 || response.statusCode === 204) {
      ...
      console.log('\n📋 Created:');
      console.log('   - google_oauth_tokens table');
      console.log('   - google_doc_id column in knowledge_base');
      console.log('   - google_doc_url column in knowledge_base');
      console.log('   - last_synced_at column in knowledge_base');
